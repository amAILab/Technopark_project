/*
  ...
  Vanilla JS: загрузка проектов и грантов из Google Sheets, фильтры, воронка, форма НТС.
*/
const CONFIG = {
...
const FUNNEL_STAGES = [
  ...
  { name: "Партнёр", hint: "Получить письмо, пилотную площадку или внешнего заказчика." },
  ...
  { name: "Отчётность / результат", hint: "Оформить результаты, акты, публикации, медиа и внедрение." },
];

// Безопасно экранируем текст из таблицы перед выводом в HTML.
...
// Читаем Google Sheets через GViz JSONP, чтобы сайт работал на GitHub Pages без секретных ключей.
function loadSheet(gid, headersRowIndex = 3) {
  return new Promise((resolve, reject) => {
    ...
    window[callback] = (payload) => {
      ...
      const headers = (rawRows[headersRowIndex] || []).map((h, i) => String(h || `col${i}`).trim());
      const data = rawRows
        .slice(headersRowIndex + 1)
...
function normalizeStatus(value) {
  ...
  if (key.includes("заверш")) return "завершён";
...
function normalizeProject(row) {
  const readiness = clampPercent(getValue(row, ["Готовность пакета", "Готовность", "readiness"]));
  return {
    id: getValue(row, ["ID", "id"]),
    name: getValue(row, ["Проект", "Название", "project"]) || "без названия",
    direction: getValue(row, ["Направление", "direction"]),
    contour: getValue(row, ["Контур", "contour"]),
    priority: getValue(row, ["Приоритет", "priority"]),
    trl: getValue(row, ["УТГ", "trl"]),
    stage: getValue(row, ["Стадия", "stage"]),
    owner: getValue(row, ["Ответственный", "owner"]),
    grant: getValue(row, ["Маршрут финансирования", "Грант", "grant"]),
    window: getValue(row, ["Ближайшее окно", "window"]),
    budget: getValue(row, ["Лимит / ориентир", "Бюджет", "budget"]),
    nextStep: getValue(row, ["Следующее действие", "Следующий шаг", "nextStep"]),
    deadline: toIsoDate(getValue(row, ["Срок", "Дедлайн", "deadline"])),
    readiness,
    status: normalizeStatus(getValue(row, ["Статус", "status"])),
    note: getValue(row, ["Блокер / примечание", "Примечание", "note"]),
  };
}

function normalizeGrant(row) {
  const windowText = getValue(row, ["Окно / статус на 27.04.2026", "Окно / статус", "window"]);
  const deadline = toIsoDate(windowText);
  return {
...
function normalizeFeedback(row) {
  return {
    id: getValue(row, ["ID"]),
    date: getValue(row, ["Дата и время"]),
    ...
    message: getValue(row, ["Текст пожелания", "Текст сообщения"]),
...
function deadlineBadge(project) {
  ...
  if (d < 0) return { label: "прошёл срок", color: "red" };
...
function renderKpi() {
  ...
  const active = state.projects.filter((p) => !["завершён", "приостановлен"].includes(p.status)).length;
  const ready = state.projects.filter((p) => (p.readiness ?? 0) >= 70 || p.status.includes("готов")).length;
  const risks = state.projects.filter(projectHasRisk).length;
  els.kpiTotal.textContent = total;
  els.kpiActive.textContent = active;
  els.kpiReady.textContent = ready;
  els.kpiRisks.textContent = risks;
}

function renderAttention() {
  const missingOwner = state.projects.filter((p) => !p.owner).slice(0, 3);
  const missingTz = state.projects.filter((p) => !normalize(p.stage).includes("тз") && !normalize(p.status).includes("тз")).slice(0, 3);
  const missingGrant = state.projects.filter((p) => !p.grant).slice(0, 3);
  const soon = state.projects.filter((p) => daysUntil(p.deadline) <= 45).sort((a, b) => daysUntil(a.deadline) - daysUntil(b.deadline)).slice(0, 3);
  const highRisk = state.projects.filter(projectHasRisk).slice(0, 3);
  const newFeedback = state.feedback.filter((f) => normalize(f.status).includes("нов") || !f.status).slice(0, 3);

  const blocks = [
    ["Проекты без ответственного", missingOwner],
    ["Проекты без ТЗ / стадии ТЗ", missingTz],
    ["Проекты без грантовой траектории", missingGrant],
    ["Ближайшие дедлайны", soon],
    ["Проекты с высоким риском", highRisk],
    ["Новые пожелания НТС", newFeedback],
  ];
...
function renderProjects() {
    ...
    return `
      <article class="project-card">
        <div class="card-top">
          ...
          <span class="badge ${statusBadge(project.status)}">${escapeHtml(project.status)}</span>
        </div>
        <div class="card-meta">
          <span class="badge blue">${escapeHtml(project.owner || "нет точных данных")}</span>
          <span class="badge ${deadline.color}">${escapeHtml(formatDate(project.deadline))}</span>
          ${projectHasUnknown(project) ? `<span class="badge yellow">нет точных данных</span>` : ""}
        </div>
  ...
  els.projectTable.innerHTML = list.map((project) => `
    <tr class="${projectHasRisk(project) ? "is-risk" : ""} ${deadlineBadge(project).color === "red" ? "is-critical" : ""}">
...
function grantStatus(grant) {
  ...
  if (deadline && deadline < CONFIG.juneStart) return { label: "раннее окно / не приоритет с июня", color: "yellow", className: "is-low-priority" };
  if (deadline && d < 0) return { label: "прошедший дедлайн", color: "gray", className: "is-archive" };
  if (deadline && d <= 21) return { label: "скоро завершится", color: "red", className: "is-low-priority" };
...
function renderGrants() {
  ...
  els.grantGrid.innerHTML = grants.map((grant) => {
    ...
    return `
      <article class="grant-card ${status.className}">
        <div class="card-top">
          <h3>${escapeHtml(grant.route)}</h3>
          <span class="badge ${status.color}">${escapeHtml(status.label)}</span>
        </div>
...
function renderFunnel() {
  ...
  els.funnelBoard.innerHTML = buckets.map((stage, index) => {
    ...
    const next = bottleneck ? "Снять неопределённость и назначить ответственного" : "Поддерживать движение к следующему этапу";
    return `
      <article class="funnel-stage">
        ...
        <div class="funnel-projects">
          ${stage.projects.slice(0, 5).map((p) => `<div class="funnel-project">${escapeHtml(p.name)}</div>`).join("") || `<div class="empty-state">пока нет проектов</div>`}
        </div>
      </article>`;
  }).join("");
}
...
function renderFeedback() {
  ...
  els.feedbackFeed.innerHTML = items.map((item) => `
    <article class="feedback-item">
      <div class="card-meta">
        <span class="badge blue">${escapeHtml(item.type || "пожелание")}</span>
        <span class="badge ${normalize(item.priority).includes("крит") || normalize(item.priority).includes("выс") ? "red" : "yellow"}">${escapeHtml(item.priority || "средний")}</span>
        <span class="badge gray">${escapeHtml(item.status || "новое")}</span>
      </div>
...
async function loadAllData() {
  ...
  try {
    const [projectRows, grantRows, feedbackRows] = await Promise.all([
      loadSheet(CONFIG.sheets.projects, 3),
      loadSheet(CONFIG.sheets.grants, 3),
      loadSheet(CONFIG.sheets.nts, 3).catch(() => []),
    ]);
    ...
    setSync("Не удалось загрузить данные. Проверьте доступ к Google Таблице или интернет-соединение.", "error");
...
async function submitFeedback(event) {
  ...
  try {
    // Используем text/plain, чтобы Apps Script принимал POST с GitHub Pages без секретных ключей.
...
// Мобильное меню и подсветка активного раздела.
